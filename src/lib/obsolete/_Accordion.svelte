<script context="module" lang="ts">
  ...
  function getSizeStyle(size: EStyleSize): [string, string, string] {
    switch (size) {
      ...
      case EStyleSize.md: return ["px-4 min-h-10", "px-4 pb-3", "w-3 h-3"];
  ...
  const flexStyle: string = "flex flex-row flex-nowrap justify-between items-center";
  ...
  import { getPartsStyle, mergeStyle, getJoinStyle, prevent, filter, sleep } from "$lib/obsolete/gfunctions";
</script>

<!---------------------------------------->

<script lang="ts">
  let { children, label, icon, isOpen = $bindable(false), group = $bindable([]), style }: Props = $props();
...
<details class={getJoinStyle(getWholeStyle(myStyle), fixedStyle)} open={open}>
  ...
  {#if isOpen}
    <div class={contentSizeStyle} transition:slide={{ duration: duration }}>
      {@render children()}
    </div>
  {/if}
</details>
